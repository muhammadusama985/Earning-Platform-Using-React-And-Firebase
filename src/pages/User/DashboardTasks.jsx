...
export default function DashboardTasks() {
  ...
  return (
<div className="min-h-screen bg-gradient-to-br from-blue-100 via-white to-blue-50 overflow-x-hidden overflow-y-auto">
{showToast && (
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-yellow-100 border border-yellow-400 text-yellow-800 px-6 py-3 rounded shadow z-50 flex flex-col items-center"
        >
          <p className="mb-2">
            Please complete the ${categoryPrices[selectedCategory]} payment to unlock {selectedCategory} tasks.
          </p>
          <button
      ...
      >
        {['Basic', 'Advance', 'Professional'].map((cat) => (
          <motion.button
            ...
            className={`px-5 py-2 rounded-full font-semibold transition-all duration-300 shadow-md flex items-center gap-1 ${
        ...
        >
          {tasks.map((task) => (
            <motion.div
              ...
              className={`bg-gradient-to-br ${categoryColors[task.category]} p-6 rounded-xl shadow-xl hover:shadow-2xl transition-all border border-blue-200`}
            >
